...
public class Antecessores extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_antecessores);
        ...
        configurarFraseDinamica();
        // Supondo que o id no seu XML seja checkboxConcluidoAntecessores
        checkboxConcluido = findViewById(R.id.checkboxConcluidoAntecessores);
        ...
        boolean jaConcluido = preferences.getBoolean("concluido_antecessores", false);
        if (checkboxConcluido != null) {
            ...
            checkboxConcluido.setOnCheckedChangeListener((buttonView, isChecked) -> {
                ...
                // 1. Salva o estado individual para a página não "esquecer" o check
                editor.putBoolean("concluido_antecessores", isChecked);

                // 2. Soma ou subtrai da contagem de JOGOS MANUAIS no Perfil
                int atualManuais = preferences.getInt("total_jogosm_concluida", 0);

                if (isChecked) {
                    editor.putInt("total_jogosm_concluida", atualManuais + 1);
                    Toast.makeText(this, "Atividade concluída com sucesso! ✨", Toast.LENGTH_SHORT).show();
                } else {
