...
public class Amigossauro extends AppCompatActivity {
    ...
    // --- CONFIGURAÇÃO DE CHAVES BRILHAKIDS ---
    private static final String PREFS_NAME = "BrilhaKidsPrefs";
    // Chave única para este vídeo específico
    private static final String KEY_CHECK_LOCAL = "concluido_amigossauro_musica";
    // Chave da CATEGORIA (Músicas = Amarelo no Perfil)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_amigossauro);
        ...
        // Carrega o vídeo da pasta res/raw
        Uri videoUri = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.amigossauro);
        ...
        // --- LÓGICA DE PROGRESSO (MÚSICAS) ---
        // Certifique-se que o ID no XML seja checkboxConcluidoSapato ou altere aqui
        checkboxConcluido = findViewById(R.id.checkboxConcluidoSapato);
        ...
        if (checkboxConcluido != null) {
            ...
            checkboxConcluido.setOnCheckedChangeListener((buttonView, isChecked) -> {
                ...
                // Atualiza a contagem na gaveta de MÚSICAS
                int totalAtual = preferences.getInt(KEY_TOTAL_MUSICAS, 0);
                if (isChecked) {
                    editor.putInt(KEY_TOTAL_MUSICAS, totalAtual + 1);
                    Toast.makeText(this, "Música concluída! Você é um Amigossauro nota 10! 🦖🎵", Toast.LENGTH_SHORT).show();
                } else {
